    ...
    while (std::getline(timestamp_file, line) && ros::ok()) {
        ...
        */
        ...
        for (std::size_t i = 0; i < lidar_data.size(); i += 4) {
            pcl::PointXYZI point;
            lidar_data[i + 3] = sqrt(lidar_data[i]*lidar_data[i] + lidar_data[i + 1]*lidar_data[i + 1]);
            if (lidar_data[i + 3] > MINIMUM_RANGE && lidar_data[i + 3] < MAXIMUM_RANGE && lidar_data[i + 2] > ZMIN_RANGE) {

...
def precompute_data_stack_mode(points, num_blocks, voxel_size, radius, neighbor_limits):
    ...
    for i in range(num_blocks):
        ...
        points_list.append(points); length_list.append(length)
...
def collate_fn_precompute_data_stack_mode(list_data, num_blocks, voxel_size, radius, neighbor_limits):
    assert num_blocks == len(neighbor_limits)
    xyz1, xyz2, feat1, feat2, trans, cov = list_data[0]
    pcd1 = precompute_data_stack_mode(xyz1, num_blocks, voxel_size, radius, neighbor_limits)
    pcd2 = precompute_data_stack_mode(xyz2, num_blocks, voxel_size, radius, neighbor_limits)
    pcd1['features'], pcd2['features'] = feat1, feat2
    return {
        'src_pcd': pcd1,
        'tar_pcd': pcd2,
        'transform': torch.from_numpy(trans.astype(np.float32)),

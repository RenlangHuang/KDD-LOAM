...
class Metrics(object):
    ...
    def relative_repeatability(self, xyz1, xyz2, trans):
        '''
        Input:  xyz1 [N, 3], xyz2 [N, 3], torch.FloatTensor
                trans [4, 4], torch.FloatTensor, groundtruth transformation matrix
        Output: relative repeatability
        '''
        xyz2 = xyz2 @ trans[:3,:3].T + trans[:3,-1:].T
        dist = torch.unsqueeze(xyz1, dim=1) - torch.unsqueeze(xyz2, dim=0)
        dist = torch.norm(dist, p=2, dim=-1).cpu().numpy()
        rp1 = np.mean(np.min(dist,axis=0)<=self.eps)
        rp2 = np.mean(np.min(dist,axis=1)<=self.eps)
        return rp1 if rp1>rp2 else rp2
